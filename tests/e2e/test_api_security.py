...
def test_api_security_invalid_key():
    response = client.post("/api/v1/workflows/fake-id/reject", headers={"X-API-Key": "wrong-key"})
    assert response.status_code == 403
...
def test_api_security_valid_key():
    ...
    response = client.post("/api/v1/workflows/fake-id/approve", headers={"X-API-Key": valid_key})
