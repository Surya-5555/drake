...
async def test_mock_httpx_executor_success(mock_async_client, mock_async_session):
    ...
    result = await executor.execute_workflow("test_wf", {"system_id": "sys123", "force": "true"})
    ...
    mock_client_instance.request.assert_any_call("GET", "http://fake/systems/sys123", timeout=5.0)
    # POST should have remaining param 'force' in JSON body
    mock_client_instance.request.assert_any_call("POST", "http://fake/systems/sys123/reset", timeout=5.0, json={"force": "true"})
