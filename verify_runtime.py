...
from src.core.database import (
    async_session, init_db, Workflow, EndpointStep
)
...
async def main():
    ...
    async with async_session() as session:
        result = await session.execute(
            select(Workflow).where(Workflow.approved == 1).options(selectinload(Workflow.steps))
        )
    ...
    with patch("src.proxy.executors.httpx_executor.httpx.AsyncClient") as MockClient:
        ...
        mock_resp_2.json.return_value = {"error": None, "message": "Successfully Completed Request"}
        mock_resp_2.text = '{"error": None, "message": "Successfully Completed Request"}'
