...
def save_csv(m: Metrics, path: Path):
    with open(path, 'w', newline='') as f:
        ...
        writer.writerow(["average_endpoints_per_workflow", round(m.average_endpoints_per_workflow, 1)])
        writer.writerow(["token_savings_percent", round(m.token_savings_percent, 1)])

def main():
    parser = argparse.ArgumentParser(description="Dell MCP Workflow Metrics Proof Engine")
    parser.add_argument("--openapi", type=Path, default=Path("openapi.json"), help="Path to OpenAPI spec file")
