...
class DeviceFactsProvider(ABC):
    ...
    """
    @abstractmethod
    async def get_device_facts(self, target_ip: str, credentials: Optional[Dict[str, Any]] = None) -> DeviceFacts:
        """
...
class RedfishFactsProvider(DeviceFactsProvider):
    ...
    async def get_device_facts(self, target_ip: str, credentials: Optional[Dict[str, Any]] = None) -> DeviceFacts:
        ...
        headers = {"Authorization": "Bearer redfish-live-facts-token", "Accept": "application/json"}
        
        async with httpx.AsyncClient(timeout=5.0) as client:
            try:
                sys_res = await client.get(f"{url}/redfish/v1/Systems/System.Embedded.1", headers=headers)
                if sys_res.status_code != 200:
                    sys_res = await client.get(f"{url}/redfish/v1/Systems/1", headers=headers)
                ...
                lc_ver = sys_data.get("Oem", {}).get("Dell", {}).get("LifecycleControllerVersion", "5.10.00.00")

                fw_inv = {}
                fw_res = await client.get(f"{url}/redfish/v1/UpdateService/FirmwareInventory", headers=headers)
                if fw_res.status_code == 200:
                    ...
                    for member in members:
                        ...
                        if ref_url:
                            item_res = await client.get(f"{url}{ref_url}", headers=headers)
...
class CachedFactsProvider(DeviceFactsProvider):
    ...
    async def get_device_facts(self, target_ip: str, credentials: Optional[Dict[str, Any]] = None) -> DeviceFacts:
        ...
        async with async_session() as session:
            ...
            if not device:
                raise ValueError(f"Device configuration cache not found for IP: {target_ip}")
...
class OMSDKFactsProvider(DeviceFactsProvider):
    ...
    """
    async def get_device_facts(self, target_ip: str, credentials: Optional[Dict[str, Any]] = None) -> DeviceFacts:
        logger.info(f"Querying hardware config dynamically via Dell OMSDK driver for IP: {target_ip}...")
