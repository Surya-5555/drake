...
from src.core.compatibility.repository import CompatibilityRepository, RiskProfileProvider, RiskProfileRepository
...
class OperationCapabilityResolver:
    ...
    async def resolve_operation_capability(self, operation_id: str) -> Optional[CapabilityInfo]:
...
class CapabilityDiscoveryService:
    ...
    async def discover_capabilities(self, endpoints: List[Dict[str, Any]]) -> List[CapabilityInfo]:
        """
        ...
        for ep in endpoints:
            ...
            existing_cap = await self.repository.get_capability_by_operation(operation_id)
            ...
            if "UpdateService.SimpleUpdate" in url or "UpdateService.Install" in url or "SimpleUpdate" in operation_id:
                domain = CompatibilityDomain.FIRMWARE
                risk_level = RiskLevel.DESTRUCTIVE
                cap_name = "FIRMWARE_UPDATE"
                profile_id = "default_destructive"
            elif "UpdateService" in url or "SoftwareInventory" in url or "FirmwareInventory" in url:
                domain = CompatibilityDomain.FIRMWARE
                risk_level = RiskLevel.READ_ONLY
                ...
                risk_level = RiskLevel.CONFIG_CHANGE if method in ("POST", "PATCH", "DELETE") else RiskLevel.READ_ONLY
                cap_name = "STORAGE_CONTROL" if method in ("POST", "PATCH", "DELETE") else "STORAGE_QUERY"
                profile_id = "default_config_change" if method in ("POST", "PATCH", "DELETE") else "default_read_only"
            elif method == "GET":
                domain = CompatibilityDomain.HARDWARE
                risk_level = RiskLevel.READ_ONLY
                cap_name = "GENERIC_QUERY"
            ...
            cap_info = CapabilityInfo(
                ...
                parameters_schema=ep.get("request_schema") if isinstance(ep.get("request_schema"), dict) else {},
    ...
    async def save_candidate_capability(self, cap_info: CapabilityInfo) -> None:
        async with self.repository.session_factory() as session:
            ...
            result = await session.execute(
                select(DBCapability).where(DBCapability.operation_id == cap_info.operation_id)
            )
...
class BlastRadiusEngine:
    ...
    async def calculate_blast_radius(self, workflow_steps: List[Any], domains: Set[str]) -> Tuple[str, str]:
        """
        Determines the containment boundary and provides a clear, explainable reason.
        """
        if "GOVERNANCE" in domains:
            return "CLUSTER", "Workflow executes platform governance operations affecting cluster-wide compliance."
        ...
        if "POWER" in domains or "STORAGE" in domains:
            ...
            if has_cross_links:
                return "RACK", "Workflow targets power/storage operations with active cross-node dependencies."
            return "CHASSIS", "Workflow modifies shared chassis components (Power/Storage controllers)."
            
        if "BIOS" in domains or "FIRMWARE" in domains:
            return "CHASSIS", "Workflow applies BIOS/Firmware updates affecting system hardware state."
            
        return "NODE", "Workflow performs read-only operations restricted to a single server node."
...
class DependencyGraphEngine:
    ...
    def verify_execution_order(self, dag: nx.DiGraph, workflow_steps: List[Any]) -> List[Tuple[str, str]]:
        """
...
class CompatibilityEngine:
    ...
    async def validate_workflow(
        ...
        logger.info(f"Evaluating compatibility of workflow {workflow_id} against IP {target_facts.target_ip}...")
        ...
        for domain in resolved_domains:
            domain_rules = await self.repository.get_rules_for_domain(domain, as_of=as_of)
        ...
        for rule in active_rules:
            ...
            if rule_type == "hardware":
                ...
                if target_op in step_operation_ids:
                    ...
                    elif step_operation_ids_list.index(prereq_op) >= step_operation_ids_list.index(target_op):
                        violations.append(CompatibilityViolation(
                            rule_id=rule_id,
                            field_checked="workflow_steps",
                            expected_value=f"Contains {prereq_op} before {target_op}",
        ...
        session_factory = getattr(self.repository, "session_factory", None) or async_session
        ...
        for idx, step in enumerate(steps):
            ...
            base_weights = {RiskLevel.READ_ONLY: 10, RiskLevel.CONFIG_CHANGE: 50, RiskLevel.DESTRUCTIVE: 90}
        ...
        blast_radius, blast_radius_reason = await blast_radius_engine.calculate_blast_radius(steps, resolved_domains)
